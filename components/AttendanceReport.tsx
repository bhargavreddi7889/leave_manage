...
import { Calendar, Clock, User, Download, Edit, Save, X, AlertCircle } from 'lucide-react'
...
export default function AttendanceReport({ 
  ...
  const [selectedUserId, setSelectedUserId] = useState<string>(userRole === 'EMPLOYEE' ? userId : '')
  ...
  const fetchAttendance = async () => {
    if (!selectedUserId && userRole === 'EMPLOYEE') {
  ...
  const canEdit = (attendance: Attendance) => {
    if (userRole === 'EMPLOYEE') return false // Employees cannot edit
    if (userRole === 'HOD') {
                    ...
                    {(userRole === 'HOD' || userRole === 'ADMIN') && (
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        {canEdit(record) ? (
                          ...
                          <span className="text-gray-400 text-xs">Cannot edit own</span>
                        )}
                      </td>
                    )}
                  </tr>
